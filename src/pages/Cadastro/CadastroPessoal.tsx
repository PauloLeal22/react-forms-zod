...
const esquemaCadastro = z.object({
  nome: z.string().min(3, "O nome tem que ter ao menos tres caracteres").transform((val) => val.split(' ').map(word => word.charAt(0).toLocaleUpperCase() + word.slice(1).toLocaleLowerCase()).join(' ')),
  email: z.string().min(1, 'Campo obrigatório').email('O e-mail não é válido').transform(valor => valor.toLocaleLowerCase()),
  telefone: z.string().regex(/^\(\d{2,3}\) \d{5}-\d{4}$/, 'O telefone é inválido'),
